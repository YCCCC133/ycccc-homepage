  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-emerald-50/50 to-white selection-primary flex flex-col">
      ...
      <div className="flex-1 flex flex-col relative">
        <div ref={chatContainerRef} className="flex-1 overflow-y-auto px-4 py-6 pb-[200px]">
              ...
              <Card className="mt-6 border-emerald-200 bg-gradient-to-br from-emerald-50/30 to-white shadow-xl shadow-emerald-500/10">
                ...
                <CardContent className="p-4">
                  ...
                  <div className="flex gap-2 mt-4">
                    <Button variant="outline" size="sm" onClick={copyToClipboard} className="flex-1 gap-2 border-emerald-200 hover:bg-emerald-50">
                      {copied ? <><Check className="h-4 w-4" /> 已复制</> : <><Copy className="h-4 w-4" /> 复制文书</>}
                    </Button>
                    <Button size="sm" onClick={downloadDocument} className="flex-1 gap-2 bg-gradient-to-r from-emerald-500 to-emerald-600 hover:from-emerald-600 hover:to-emerald-700 shadow-lg shadow-emerald-500/30">
            ...
            <div className="container mx-auto max-w-3xl px-4 py-4">
              {currentStep > 0 && (
                <div className="flex gap-2 mb-3 text-xs">
                  <button onClick={() => handleAnswer('跳过')} className="px-3 py-1 rounded-full bg-gray-100 hover:bg-gray-200 text-gray-600">跳过</button>
                  <button onClick={() => handleAnswer('重新开始')} className="px-3 py-1 rounded-full bg-gray-100 hover:bg-gray-200 text-gray-600">重新开始</button>
                  <button onClick={() => handleAnswer('帮助')} className="px-3 py-1 rounded-full bg-gray-100 hover:bg-gray-200 text-gray-600">帮助</button>
                </div>
              )}
              ...
              <form onSubmit={handleSubmit} className="flex gap-2">
                ...
                <Button type="submit" size="icon" disabled={isGenerating || isTyping || !inputValue.trim()}
              ...
              {currentQ?.options && (
                <div className="flex gap-2 mt-3 justify-center">
                  {currentQ.options.map(opt => (
                    <Button key={opt} variant="outline" size="sm" onClick={() => handleAnswer(opt)}
                      className="text-xs border-emerald-200 hover:bg-emerald-50">{opt}</Button>
                  ))}
                </div>
              )}
              ...
              {currentStep > 0 && (
                <div className="mt-3 flex items-center gap-2">
                  ...
                  <span className="text-xs text-muted-foreground shrink-0">{currentStep}/{QUESTIONS.length}</span>
                </div>
              )}
            </div>
          </div>
